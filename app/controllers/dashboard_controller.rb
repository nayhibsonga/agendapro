class DashboardController < ApplicationController
  ...
  layout "admin"
	...
	def index
		...
		@todayBookings = Booking.where(service_provider_id: @service_providers).where.not(status_id: Status.find_by_name("Cancelado")).where("DATE(start) = DATE(?)", Time.now - eval(ENV["TIME_ZONE_OFFSET"])).order(:start)
		@monthBookings = Booking.where(service_provider_id: @service_providers).where("created_at BETWEEN ? AND ?", Time.now.beginning_of_month - eval(ENV["TIME_ZONE_OFFSET"]), Time.now.end_of_month - eval(ENV["TIME_ZONE_OFFSET"]))
		@statusArray = []
