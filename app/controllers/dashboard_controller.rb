class DashboardController < ApplicationController
  ...
  layout "admin"
	...
	def index
		...
		@todayBookings = Booking.where(service_provider_id: @service_providers).where.not(status_id: Status.find_by_name("Cancelado")).where("DATE(start) = DATE(?)", Time.now).order(:start)
