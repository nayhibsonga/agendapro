class ClientsController < ApplicationController
  ...
  before_action :verify_blocked_status, except: [:history, :bookings_history, :check_sessions, :suggestion, :name_suggestion, :rut_suggestion, :show, :new, :edit, :create, :update, :update_custom_attributes, :check_sessions, :get_custom_attributes, :client_loader, :last_payments]
  before_action :verify_premium_plan, only: [:files, :create_folder, :rename_folder, :delete_folder, :upload_file, :move_file, :edit_file]
  before_action :verify_disabled, only: [:index, :show, :new, :edit]
  ...
  def bookings_history
    ...
    last_bookings = []
    last_creations = []
    client.bookings.where('start < ?', Time.now).where('is_session = false or (is_session = true and is_session_booked = true)').order(start: :desc).limit(5).each do |booking|
      last_bookings.push( { start: booking.start, created_at: (booking.created_at + timezone_offset.hours), service: booking.service.name, provider: booking.service_provider.public_name, status: booking.status.name, notes: booking.notes, comment: booking.company_comment } )
    end
    client.bookings.where('is_session = false or (is_session = true and is_session_booked = true)').order('created_at desc').limit(5).each do |booking|
      last_creations.push( { start: booking.start,  created_at: (booking.created_at + timezone_offset.hours), service: booking.service.name, provider: booking.service_provider.public_name, status: booking.status.name, notes: booking.notes, comment: booking.company_comment } )
    end

    bookings = {last_bookings: last_bookings, last_creations: last_creations}
  ...
  def create_comment
    ...
    @client_comment.user_id = current_user.id
    respond_to do |format|
  ...
  def update_comment
    ...
    @client_comment.last_modifier_id = current_user.id
  ...
  #Given a client and a service, look por payments that have mock_bookings with that service
  #It is intended for booking to payment association with existing payments
  def past_payments
    ...
    @payments = @client.payments.where(id: @client.mock_bookings.where(service_id: @service.id).pluck(:payment_id))

    respond_to do |format|
      format.html { render :partial => 'past_payments' }
    end
