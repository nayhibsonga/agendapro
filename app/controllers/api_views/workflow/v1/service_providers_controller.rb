    ...
    module V1
      class ServiceProvidersController < V1Controller
        ...
        def available_days
          ...
          sd.upto(ed) do |date|
            if date <= Time.now - 1.days
              available = false
            else
              available = true
            end
            wd = date.wday == 0 ? 7 : date.wday
            serviceStaff.each do |service_staff|
              if available && service_staff[:service].present? && service_staff[:provider].present? && params[:local].present?
                service_provider = service_staff[:provider] == "0" ? ServiceProvider.where(location_id: params[:local], active: true, online_booking: true) : service_staff[:provider]
                if ProviderTime.where(day_id: wd, service_provider_id: service_provider).count > 0
            ...
            @available_days.push({date: date, available: available})
          end
