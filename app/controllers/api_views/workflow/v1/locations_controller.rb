	  ...
	  class LocationsController < V1Controller
	  	...
	  	def show
	  		...
	  		@long_days = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo']
				...
				@attributes = @location.company.custom_attributes.where(show_on_workflow: true).joins(:attribute_group).order('"attribute_groups"."order", "attribute_groups"."name", "attributes"."order", "attributes"."name"')
		end
	end
end
end
