class ServicesController < ApplicationController
  ...
  def show_time_promo
    ...
    if @relatedPromos.count < 6
      ...
      query = Location.where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(:id => CompanyEconomicSector.where(economic_sector_id: @company.economic_sectors).pluck(:company_id)).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true, id: PromoTime.where(:active => true).pluck(:company_setting_id)).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @location.latitude.to_s + ')^2 + (longitude - ' + @location.longitude.to_s + ')^2) < 0.25')
