class ServiceProvidersController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      format.html
      format.pdf do
        pdf = ServiceProvidersPdf.new(@service_provider, date)
        send_data pdf.render, filename: @service_provider.public_name + "_" + date.to_s + '.pdf', type: 'application/pdf'
      end
    end
  end
  ...
  private
    ...
    def service_provider_params
      params.require(:service_provider).permit(:user_id, :location_id, :public_name, :notification_email, :block_length, :service_ids => [], provider_times_attributes: [:id, :open, :close, :day_id, :service_provider_id, :_destroy], user_attributes: [:email, :password, :confirm_password, :role_id, :company_id, :location_id])
    end
end
