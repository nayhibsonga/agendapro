class ServiceProvidersController < ApplicationController
  before_action :set_service_provider, only: [:show, :edit, :update, :destroy, :activate, :deactivate, :default_time, :is_open]
  ...
  def location_providers
    render :json => ServiceProvider.actives.where('location_id = ?', params[:location]).accessible_by(current_ability).order(:order, :public_name)
  end
  ...
  def show_open_day
    ...
    open_day = ProviderOpenDay.find(params[:provider_open_day_id])
    @location = open_day.service_provider.location
    date = open_day.start_time.to_datetime
    ...
    if @location.location_times.where(day_id: date.cwday).count > 0
      min_hour = @location.location_times.where(day_id: date.cwday).first.open.strftime("%R")
      max_hour = @location.location_times.where(day_id: date.cwday).first.close.strftime("%R")
    else
      if @location.location_open_days.where('start_time >= ? AND start_time <= ?', day_start, day_end).count > 0
    ...
    render :json => {id: open_day.id, date: open_day.start_time.strftime('%d/%m/%Y'), start_hour: open_day.start_time.strftime('%H'), start_minute: open_day.start_time.strftime('%M'), end_hour: open_day.end_time.strftime('%H'), end_minute: open_day.end_time.strftime('%M'), min_hour: min_hour, max_hour: max_hour}
  end

  def create_open_day
    open_day = ProviderOpenDay.new(service_provider_id: params[:service_provider_id], start_time: params[:start_time].to_datetime, end_time: params[:end_time].to_datetime)
    if open_day.save
  ...
  def update_open_day
    open_day = ProviderOpenDay.find(params[:provider_open_day_id])
    if open_day.update(start_time: params[:start_time].to_datetime, end_time: params[:end_time].to_datetime)
      render :json => {status: "ok", open_day: open_day, date: l(open_day.start_time.to_date, format: :long), start_time: open_day.start_time.strftime("%H:%M"), end_time: open_day.end_time.strftime("%H:%M")}
    else
      render :json => {status: "error", errors: open_day.errors.full_messages}
    end
  ...
  def is_open
    ...
    render :json => {day_open: @service_provider.is_open(start_time, end_time)}
  end
  ...
  private
    ...
    def service_provider_params
      params.require(:service_provider).permit(:user_id, :location_id, :public_name, :block_length, :online_booking, :booking_leap, :service_ids => [], provider_times_attributes: [:id, :open, :close, :day_id, :service_provider_id, :_destroy], user_attributes: [:email, :password, :confirm_password, :role_id, :company_id, :location_id], _provider_open_days_attributes: [:service_provider_id, :start_time, :end_time])
    end
end
