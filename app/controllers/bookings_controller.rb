class BookingsController < ApplicationController
  ...
  force_ssl if: :ssl_configured?, only: [:index, :update, :destroy]
...
>>>>>>> mailing2
                    ...
                    logger.info "Booking delete: #{discharged_booking.id}. Reason: Admin modification of a booked session (should be restored with another id). (1)"
              ...
              if @booking.is_session || @booking.service.has_sessions
                if !booking_params[:session_booking_id].blank? && booking_params[:session_booking_id].to_i != 0
                  ...
                  if !session_booking.nil?
                    ...
                    discharged_booking = session_booking.bookings.where(is_session_booked: false).where.not(id: @booking.id).last
                    logger.info "Booking delete: #{discharged_booking.id}. Reason: Admin modification of a booked session (should be restored with another id). (2)"
              ...
              if @booking.is_session || @booking.service.has_sessions
                ...
                if !booking_params[:session_booking_id].blank? && booking_params[:session_booking_id].to_i != 0
                  ...
                  if !session_booking.nil?
                    ...
                    #There is session_booking, book a session, unbook for old_treatment
                    discharged_booking = session_booking.bookings.where(is_session_booked: false).where.not(id: @booking.id).last
                    logger.info "Booking delete: #{discharged_booking.id}. Reason: Admin modification of a booked session (should be restored with another id). (3)"
...
>>>>>>> mailing2
  ...
  def update_book_session
    ...
    @booking.is_session_booked = true
    @booking.user_session_confirmed = true
    @booking.status_id = Status.find_by_name("Reservado").id
    ...
    @providers.each do |provider|
      ...
      while current_date < end_date
        if current_date.wday == 0
          ...
          current_day = current_date.cwday
        end

        day_start = current_date.beginning_of_day
        day_end = current_date.end_of_day
        #Check if location is open
        #If open, get start and end from it's time
        #If not, get max start/end location time difference for the event
        # if provider.location.location_times.where(day_id: current_day).count > 0
        #   location_time = provider.location.location_times.where(day_id: current_day).first
        #   day_start = day_start.change(hour: location_time.open.hour, min: location_time.open.min)
        #   day_end = day_end.change(hour: location_time.close.hour, min: location_time.close.min)
        # else
        #   if provider.location.location_times.count > 0
        #     location_time = provider.location.location_times.order('(close - open) desc').first
        #     day_start = day_start.change(hour: location_time.open.hour, min: location_time.open.min)
        #     day_end = day_end.change(hour: location_time.close.hour, min: location_time.close.min)
        #   end
        # end

        provider.provider_times.where(day_id: current_day).order(:open).each do |provider_time|
          logger.debug "Enters for provider_time #{provider_time.id}"
          event = Hash.new
          event_end = day_start.change(hour: provider_time.open.hour, min: provider_time.open.min)
          if day_start != event_end

            #Check for existing open_days and split the event when necessary

            #If there's no open_day, just add the event
            open_days_count = provider.provider_open_days.where('start_time >= ? and end_time <= ?', day_start, event_end).count
            if open_days_count == 0
              event = {
                ...
                editable: false,
                start: day_start,
                end: day_start.change(hour: provider_time.open.hour, min: provider_time.open.min),
                resourceId: provider.id,
                className: 'blocked',
                start_time: day_start.strftime("%R"),
                end_time: day_start.change(hour: provider_time.open.hour, min: provider_time.open.min).strftime("%R"),
                event_date: day_start.strftime("%Y-%m-%d")
              }
              events.push(event)
            else
              #Loop through overlapping open_days to split the blocked_event into pieces
              #If it's the last open_day in the interval and there is a gap between open_day's end and event's end, add a closing blocked event
              provider.provider_open_days.where('start_time >= ? and end_time <= ?', day_start, event_end).order(:start_time).each_with_index do |open_day, index|
                #If starts don't match, then time is blocked between them
                if day_start != open_day.start_time && day_start != event_end
                  event = {
                    id: 'p'+provider_time.id.to_s,
                    title: 'Bloqueo por Horario',
                    allDay: false,
                    editable: false,
                    start: day_start,
                    end: open_day.start_time,
                    resourceId: provider.id,
                    className: 'blocked',
                    start_time: day_start.strftime("%R"),
                    end_time: open_day.start_time.strftime("%R"),
                    event_date: day_start.strftime("%Y-%m-%d")
                  }
                  events.push(event)
                  day_start = day_start.change(hour: open_day.end_time.hour, min: open_day.end_time.min)

                  #Fill the gap if necessary
                  if index == (open_days_count - 1) && day_start != event_end
                    event = {
                      id: 'p'+provider_time.id.to_s,
                      title: 'Bloqueo por Horario',
                      allDay: false,
                      ...
                      className: 'blocked',
                      start_time: day_start.strftime("%R"),
                      end_time: event_end.strftime("%R"),
                      event_date: day_start.strftime("%Y-%m-%d")
                    }
                    events.push(event)
                    day_start = day_start.change(hour: event_end.hour, min: event_end.min)
                  end

                end
              end
            end
            day_start = day_start.change(hour: provider_time.close.hour, min: provider_time.close.min)
          else
            day_start = day_end
          end
          logger.debug "Day start: "
          logger.debug day_start.to_s
        end
        if day_start < day_end
          logger.debug "No provider_time, gonna check for open_days"
          open_days_count = provider.provider_open_days.where('start_time >= ? and end_time <= ?', day_start, day_end).count
          if open_days_count == 0
            event = Hash.new
            event = {
              id: 'pp'+provider.id.to_s,
              ...
              className: 'blocked',
              start_time: day_start.strftime("%R"),
              end_time: day_end.strftime("%R"),
              event_date: day_start.strftime("%Y-%m-%d")
            }
            events.push(event)
          else

            provider.provider_open_days.where('start_time >= ? and end_time <= ?', day_start, day_end).order(:start_time).each_with_index do |open_day, index|
              #If starts don't match, then time is blocked between them
              if day_start != open_day.start_time && day_start != day_end
                event = {
                  id: 'pp'+provider.id.to_s,
                  title: 'Bloqueo por Horario',
                  allDay: false,
                ...
                #Fill the gap if necessary
                if index == (open_days_count - 1) && day_start != day_end
                  event = {
                    id: 'pp'+provider.id.to_s,
                    title: 'Bloqueo por Horario',
                    allDay: false,
                  ...
                  events.push(event)
                  day_start = day_start.change(hour: day_end.hour, min: day_end.min)
                end
  ...
  def book_service
    ...
    if params[:location].blank?
      ...
      redirect_to localized_search_path
      return
  ...
  def edit_booking
    ...
    if mobile_request?
      ...
      providers = ServiceProvider.where(id: @booking.service_provider_id)

      @available_time = []

      if location_times.length > 0 || @location.location_open_days.where('start_time >= ? AND start_time <= ?', @date.beginning_of_day.change(:format => "+0000"), @date.end_of_day.change(:format => "+0000")).count > 0

        start_date = @date.beginning_of_day.change(:format => "+0000")
        end_date = @date.end_of_day.change(:format => "+0000")

        ActiveRecord::Base.connection.execute("SELECT * FROM available_hours(#{company_id}, #{@location.id}, ARRAY#{[@booking.service_provider_id]}, ARRAY#{[@service.id]}, ARRAY#{[0]}, '#{start_date}', '#{end_date}', false, ARRAY#{providers.pluck(:id)})").each do |row|
  ...
  def optimizer_hours
    ...
    if params[:start_date] && params[:start_date] != ""
      ...
      logger.debug "Date: #{current_date}"
    else
    ...
    while @hours_array.length < array_length
      ...
      if date > DateTime.now + company_setting.after_booking.months
        break
      end
      ...
      if dtp.nil?
        if local.location_open_days.where('start_time >= ? AND start_time <= ?', date, date.end_of_day).count == 0
          date = date + 1.days
          next
        end
      end
      ...
      if loc_close.nil?
        if local.location_open_days.where('start_time >= ? AND start_time <= ?', start_date, start_date.end_of_day).count > 0
          open_day = local.location_open_days.where('start_time >= ? AND start_time <= ?', start_date, start_date.end_of_day).order(:end_time).last
          end_date = DateTime.new(start_date.year, start_date.mon, start_date.mday, open_day.end_time.hour, open_day.end_time.min)
        else
          next
        end
      else
      ...
      #Go to next day if array not complete
      if @hours_array.length < array_length
        date = date + 1.days
      end
  ...
  def hours_test
    first_date = "2016-04-18T00:00:00-03:00".to_date.beginning_of_day
    json_response = []
  ...
  def available_hours
    ...
    for i in 0..serviceStaff.length-1
      services_arr[i] = serviceStaff[i][:service].to_i
      cur_serv = Service.find(services_arr[i])
      total_services_duration += cur_serv.duration
      services << cur_serv
      bundles_arr[i] = serviceStaff[i][:bundle_id].nil? ? 0 : serviceStaff[i][:bundle_id].to_i
      providers_arr[i] = serviceStaff[i][:provider].to_i
    end
    ...
    leap_duration_diff = 0
    if booking_leap > total_services_duration
      leap_duration_diff = booking_leap - total_services_duration
    end
    ...
    if !company_setting.allows_overlap_hours || company_setting.allows_optimization
      ...
      if leap_duration_diff == 0
        time_prop = hours_diff/booking_leap
        calendar_height = time_prop*67
        adjusted_calendar_height = calendar_height
      else
        time_prop = hours_diff.to_f/booking_leap.to_f
        aux_time_prop = time_prop

        calendar_height = time_prop * 67
        aux_m = (leap_duration_diff.to_f / booking_leap.to_f) * 67

        adjusted_calendar_height = aux_time_prop * (67 + aux_m) # + (calendar_height.to_f * leap_duration_diff.to_f/(60.to_f * hours_diff.to_f) ) * 67
      end
    end
          ...
          if hours_diff != 0
            ...
            if company_setting.allows_overlap_hours && !company_setting.allows_optimization
              ...
              if leap_duration_diff == 0
                if hour[:start_block].to_time.strftime("%H:%M") > (previous_start.to_time + booking_leap.minutes).strftime("%H:%M")
                  ...
                  if previous_start == min_block_str && previous_hour == min_block_str
            ...
            local.location_times.each do |lt|
              if new_max_close == 0
                ...
                if lt.close.strftime("%H:%M") > new_max_close.to_s
  ...
  def associate_payment
    ...
    @mock_booking = MockBooking.find(params[:mock_booking_id])

    if @booking.update(payment_id: @payment.id, receipt_id: @mock_booking.receipt_id, payed_state: true, price: @mock_booking.price)
      @mock_booking.destroy
      ...
      if @booking.is_session && !@booking.session_booking.nil?
        sessions_ratio = "Sesión " + (@booking.session_booking.bookings.where(is_session_booked: true).where('start < ?', @booking.start).count + 1).to_s + " de " + @booking.session_booking.sessions_amount.to_s
      end
      ...
      render :json => {status: "ok", booking: @booking_json}
    else
