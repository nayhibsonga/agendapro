class PlansController < ApplicationController
  ...
  def select_plan
    ...
    @company.plan == Plan.find_by_name("Trial") ? @price = Plan.where(locations: @company.locations.where(active: true).count).where('service_providers <= ?', @company.service_providers.where(active: true).count).first.price : @price = @company.plan.price
    ...
    @company.months_active_left > 0 ? @plan_1 = (@company.due_amount + @price*(1+@sales_tax)).round(0) : @plan_1 = ((@company.due_amount + (@month_days - @day_number)*@price/@month_days)*(1+@sales_tax)).round(0)
