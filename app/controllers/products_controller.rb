class ProductsController < ApplicationController
  ...
  def locations_stats
    ...
    @company.service_providers.where(active: true).each do |service_provider|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]).pluck(:id), seller_type: 0, seller_id: service_provider.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
    ...
    @company.users.each do |user|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]).pluck(:id), seller_type: 1, seller_id: user.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
    ...
    @company.employee_codes.each do |employee_code|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]).pluck(:id), seller_type: 2, seller_id: employee_code.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
  ...
  def locations_stats_excel
    ...
    @company.service_providers.where(active: true).each do |service_provider|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids].split(",")).pluck(:id), seller_type: 0, seller_id: service_provider.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
      provider_tuple = [service_provider, product_price_sum, product_quantity_sum]
    ...
    @company.users.each do |user|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids].split(",")).pluck(:id), seller_type: 1, seller_id: user.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
      user_tuple = [user, product_price_sum, product_quantity_sum]
    ...
    @company.employee_codes.each do |employee_code|
      product_price_sum = 0
      product_quantity_sum = 0
      PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids].split(",")).pluck(:id), seller_type: 2, seller_id: employee_code.id).where(product_id: products.pluck(:id)).each do |pp|
        product_quantity_sum += pp.quantity
        product_price_sum += pp.quantity * pp.price
      end
      employee_code_tuple = [employee_code, product_price_sum, product_quantity_sum]
  ...
  def seller_history
    ...
    if @products_status == "inactive"
      @payment_products = PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]), seller_type: params[:seller_type], seller_id: params[:seller_id]).where(product_id: Product.where(company_id: current_user.company_id, active: false).pluck(:id)).order('(quantity * price) desc')
    elsif @products_status == "active"
      @payment_products = PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]), seller_type: params[:seller_type], seller_id: params[:seller_id]).where(product_id: Product.where(company_id: current_user.company_id, active: true).pluck(:id)).order('(quantity * price) desc')
    else
  ...
  def product_history
    ...
    if @products_status == "inactive"
      @payment_products = PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]), product_id: params[:product_id]).where(product_id: Product.where(company_id: current_user.company_id, active: false).pluck(:id)).order('(quantity * price) desc')
    elsif @products_status == "active"
      @payment_products = PaymentProduct.where(payment_id: Payment.where(payment_date: @from..@to, location_id: params[:location_ids]), product_id: params[:product_id]).where(product_id: Product.where(company_id: current_user.company_id, active: true).pluck(:id)).order('(quantity * price) desc')
    else
  ...
  def logs_history
    ...
    @product_logs = ProductLog.where(location_id: @locations.pluck(:id), product_id: @products.pluck(:id), created_at: @from.beginning_of_day..@to.end_of_day)
  ...
  def logs_history_excel
    ...
    @company = current_user.company
    @locations = Location.where(id: params[:location_ids].split(","))
