module Api
  module V1
      ...
      def bookings
        ...
        @activeBookings = Booking.where('is_session = false or (is_session = true and is_session_booked = true)').where(:client_id => @client_ids, :status_id => Status.where(:name => ['Reservado' 'Confirmado'])).where("start > ?", DateTime.now + timezone.offset.hours).order(:start).group_by{ |i| i.start.to_date }
