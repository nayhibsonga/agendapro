  ...
  module V1
      ...
      def index
        if params[:latitude].present? && params[:longitude].present?
      	...
      	@economic_sectors = EconomicSector.select('economic_sectors.id, economic_sectors.name, economic_sectors.mobile_preview, count(company_economic_sectors.id) AS companies_count').joins(:company_economic_sectors).where(company_economic_sectors: { company_id: @locations.pluck(:id).uniq } ).group('economic_sectors.id').having('count(company_economic_sectors.id) > ?', 0).order('companies_count DESC')
