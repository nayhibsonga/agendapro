module Api
  module V1
      ...
      def index
      	...
      	@economic_sectors = EconomicSector.select('economic_sectors.id, economic_sectors.name, economic_sectors.mobile_preview, count(company_economic_sectors.id) AS companies_count').joins(:company_economic_sectors).where(company_economic_sectors: { company_id: @locations.pluck(:id).uniq } ).group('economic_sectors.id').order('companies_count DESC')
