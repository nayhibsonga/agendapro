			...
			@longitude = params[:longitude]
      		if params[:search_text].present?
				...
				@results = Array.new
					...
					results.each do |result|
						if Location.find(result['location_id']) && Location.where(company_id: Company.where(id: CompanyEconomicSector.where(economic_sector_id: EconomicSector.where(marketplace: true)).pluck(:company_id))).pluck(:id).include?(result['location_id'])
