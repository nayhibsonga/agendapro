class  Admin::UsersController < ApplicationController
  ...
  # GET /users.json
  def index
    @users = User.where.not(email: 'cuentas@agendapro.cl').where(company_id: current_user.company_id).accessible_by(current_ability)
  end
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:id, :first_name, :last_name, :email, :phone, :password, :role_id, :company_id, :location_ids => [], :service_provider_ids => [] )
    end
end
