class QuickAddController < ApplicationController
 ...
 #        	format.json { render :layout => false, :json => {:valid => true} }
  ...
  # 	end
  	...
  	def location_params
      params.require(:location).permit(:name, :address, :second_address, :phone, :longitude, :latitude, :company_id, :outcall, :outcall_places, :email, location_times_attributes: [:id, :open, :close, :day_id, :location_id, :_destroy])
    end
