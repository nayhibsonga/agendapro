class SurveyAnswersController < ApplicationController
  ...
  def index
    ...
    SurveyAnswer.where(booking_id: Booking.where(location_id: Location.where(company_id: current_user.company_id).pluck(:id))).each do |eva|
  ...
  def average
    surveys = Booking.where(location_id: Location.where(company_id: current_user.company_id).pluck(:id)).where.not(survey_id:nil).where(survey_id: Survey.where.not(appreciation: nil).pluck(:id))
    @average = 0
    surveys.each do |s|
