...
class SearchsController < ApplicationController
	...
	def search2
		if params[:inputSearch] && params[:latitude] && params[:longitude] && params[:inputLocalization]
			...
			economic_sectors = EconomicSector.joins(:companies).where("companies.id" => Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true))

			empty_economic_sectors = EconomicSector.joins(:companies).where("companies.id" => Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => false))
			...
			companies = Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true)
			...
			#Iteramos sobre las compañías porque un local sólo tiene una compañía
			companies = Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => false)
			companies.each do |company|
	...
	def promotions
			...
			if normalized_search != ""
				...
				time_query = Location.search_services(normalized_search).where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true, id: PromoTime.where(:active => true).pluck(:company_setting_id)).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')

			else

				time_query = Location.where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true, id: PromoTime.where(:active => true).pluck(:company_setting_id)).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')
			...
			if normalized_search != ""
				...
				last_minute_query = Location.search_services(normalized_search).where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')

			else

				last_minute_query = Location.where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')
	...
	def last_minute_promotions
		...
		if normalized_search != ""
				...
				query = Location.search_services(normalized_search).where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')

			else

				query = Location.where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')
	...
	def treatment_promotions
			...
			if normalized_search != ""
				...
				treatment_query = Location.search_services(normalized_search).where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true, id: PromoTime.where(:active => true).pluck(:company_setting_id)).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')

			else

				treatment_query = Location.where(company_id: Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(:owned => true).where(id: CompanySetting.where(:activate_search => true, :activate_workflow => true, id: PromoTime.where(:active => true).pluck(:company_setting_id)).pluck('company_id'))).where(:active => true).where('sqrt((latitude - ' + @latitude.to_s + ')^2 + (longitude - ' + @longitude.to_s + ')^2) < 0.25')
