class LocationsController < ApplicationController
  before_action :set_location, only: [:show, :edit, :update, :destroy, :activate, :deactivate, :sellers, :is_open]
  ...
  def index
    if current_user.role == Role.find_by(name: "Super Admin")
      @companies = Company.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id).where(owned: true).order(payment_status_id: :desc)
    else
    ...
    if current_user.role == Role.find_by(name: 'Super Admin')
      ...
      respond_to do |format|
        if @location.update(location_params)
          ...
          #Adjust location and providers open_days
          @location.adjust_times
          flash[:success] = 'Local actualizado exitosamente.'
          flash.keep(:success)
          format.html { redirect_to locations_path }
          format.json { render :json => location }
  ...
  def inventory
    ...
    if normalized_search != ""
      if params[:status] == "active"
        location_products = @location.location_products.where(product_id: @location.company.products.where(active: true).pluck(:id)).search(normalized_search)
      else
        location_products = @location.location_products.where(product_id: @location.company.products.where(active: false).pluck(:id)).search(normalized_search)
      end
    else
      if params[:status] == "active"
        location_products = @location.location_products.where(product_id: @location.company.products.where(active: true).pluck(:id))
      else
        location_products = @location.location_products.where(product_id: @location.company.products.where(active: false).pluck(:id))
      end
    end
  ...
  def show_open_day
    open_day = LocationOpenDay.find(params[:location_open_day_id])
    render :json => {id: open_day.id, date: open_day.start_time.strftime('%d/%m/%Y'), start_hour: open_day.start_time.strftime('%H'), start_minute: open_day.start_time.strftime('%M'), end_hour: open_day.end_time.strftime('%H'), end_minute: open_day.end_time.strftime('%M')}
  end

  def create_open_day
    open_day = LocationOpenDay.new(location_id: params[:location_id], start_time: params[:start_time], end_time: params[:end_time])
    if open_day.save
      render :json => {status: "ok", open_day: open_day, date: l(open_day.start_time.to_date, format: :long), start_time: open_day.start_time.strftime("%H:%M"), end_time: open_day.end_time.strftime("%H:%M")}
    else
      render :json => {status: "error", errors: open_day.errors.full_messages}
    end
  end

  def update_open_day
    open_day = LocationOpenDay.find(params[:location_open_day_id])
    if open_day.update(start_time: params[:start_time], end_time: params[:end_time])
      render :json => {status: "ok", open_day: open_day, date: l(open_day.start_time.to_date, format: :long), start_time: open_day.start_time.strftime("%H:%M"), end_time: open_day.end_time.strftime("%H:%M")}
    else
      render :json => {status: "error", errors: open_day.errors.full_messages}
    end
  ...
  def delete_open_day
    open_day = LocationOpenDay.find(params[:location_open_day_id])
    if open_day.destroy
      render :json => {status: "ok", open_day: open_day}
    else
      render :json => {status: "error", errors: open_day.errors.full_messages}
    end
  end
  ...
  def is_open
    ...
    logger.debug "Day start: #{day_start}"
    logger.debug "Day end: #{day_end}"

    if @location.location_times.where(day_id: date.cwday).count > 0
      correct_time = true
      start_time = @location.location_times.where(day_id: date.cwday).first.open.strftime("%R")
      end_time = @location.location_times.where(day_id: date.cwday).first.close.strftime("%R")
    else
      if @location.location_open_days.where('start_time >= ? AND start_time <= ?', day_start.change(:offset => "+0000"), day_end.change(:offset => "+0000")).count > 0
        correct_time = true
        start_time = @location.location_open_days.where('start_time >= ? AND start_time <= ?', day_start.change(:offset => "+0000"), day_end.change(:offset => "+0000")).order(:start_time).first.start_time.strftime("%R")
        end_time = @location.location_open_days.where('start_time >= ? AND start_time <= ?', day_start.change(:offset => "+0000"), day_end.change(:offset => "+0000")).order(end_time: :desc).first.end_time.strftime("%R")
      end
    end

    render :json => {day_open: correct_time, start_time: start_time, end_time: end_time, role: role}
  end
