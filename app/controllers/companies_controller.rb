class CompaniesController < ApplicationController
  before_action :verify_is_active, only: [:overview, :workflow]
	...
	layout "admin", except: [:show, :overview, :workflow, :add_company, :select_hour, :user_data, :select_session_hour, :select_promo_hour, :mobile_hours]
			  ...
			  if params[:mandatory_discount]
			    ...
			    if has_time_discount
			      ...
			      new_hour = {
			        ...
			        discount: bookings_group_discount.to_s,
			        time_discount: bookings_group_discount.to_s,
			        service_promo_id: Service.find(serviceStaff[0][:service]).active_service_promo_id
			      }
			  	...
			  	hour = {
			  		:start => I18n.l(bookings[0][:start].to_datetime, format: :hour),
			  		:end => I18n.l(bookings[bookings.length - 1][:end].to_datetime, format: :hour)
			  	}
