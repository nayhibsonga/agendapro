class CompaniesController < ApplicationController
	...
	def incomes
		...
		if(!params[:filter].blank?)
			if params[:filter] == "active"
				@filter = "active"
				@companies = @companies.where.not(payment_status_id: PaymentStatus.find_by_name("Inactivo").id)
			elsif params[:filter] == "inactive"
				@filter = "inactive"
				@companies.where(payment_status_id: PaymentStatus.find_by_name("Inactivo").id)
			end
		end
	...
	def overview
		...
		unless @company.company_setting.activate_workflow && @company.payment_status_id != PaymentStatus.find_by_name("Inactivo").id && @locations.uniq.count > 0
	...
	def workflow
		...
		if @company.plan_id == Plan.find_by_name("Gratis").id
      		redirect_to localized_search_path, error: "Esta compañía no tiene minisitio."
      		return
    	end

		unless @company.company_setting.activate_workflow && @company.payment_status_id != PaymentStatus.find_by_name("Inactivo").id
			flash[:alert] = "Lo sentimos, el mini-sitio que estás buscando no se encuentra disponible."
	...
	def inventory
	    ...
	    if normalized_search != ""
	    	if params[:status] == "active"
	      		products = @company.products.where(active: true).search(normalized_search)
	      	else
	      		products = @company.products.where(active: false).search(normalized_search)
	      	end
	    else
	    	if params[:status] == "active"
	      		products = @company.products.where(active: true)
	      	else
	      		products = @company.products.where(active: false)
	      	end
	    end
		...
		def mobile_hours(serviceStaff)
			...
			if dtp.nil?
				if local.location_open_days.where('start_time >= ? AND start_time <= ?', @date.beginning_of_day.change(:format => "+0000"), @date.end_of_day.change(:format => "+0000")).count > 0
					dtp = local.location_open_days.where('start_time >= ? AND start_time <= ?', @date.beginning_of_day.change(:format => "+0000"), @date.end_of_day.change(:format => "+0000")).first.start_time
				end
			else
				dtp = dtp.open
			end
