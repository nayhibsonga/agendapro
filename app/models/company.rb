class Company < ActiveRecord::Base
	...
	def self.end_trial
		...
		where(active: true, payment_status_id: PaymentStatus.find_by_name("Trial").id).where.not(plan_id: Plan.find_by_name("Gratis").id).where('created_at <= ?', 1.days.ago).each do |company|
