class Company < ActiveRecord::Base
	...
	scope :collectables, -> { where('created_at <= ?', DateTime.now - 2.months).where.not(plan_id: Plan.where(name: ["Gratis", "Trial"]).pluck(:id)).where.not(payment_status_id: PaymentStatus.where(name: ["Inactivo", "Bloqueado", "Admin", "Convenio PAC"]).pluck(:id)) }
	scope :former_trials, -> { where('created_at > ?', DateTime.now - 2.months).where.not(plan_id: Plan.where(name: ["Gratis", "Trial"]).pluck(:id)).where.not(payment_status_id: PaymentStatus.where(name: ["Inactivo", "Bloqueado", "Admin", "Convenio PAC"]).pluck(:id)) }
	scope :regular_blocked, -> { where('created_at <= ?', DateTime.now - 2.months).where.not(plan_id: Plan.where(name: ["Gratis", "Trial"]).pluck(:id)).where(payment_status_id: PaymentStatus.find_by_name("Bloqueado").id) }
