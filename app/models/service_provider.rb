class ServiceProvider < ActiveRecord::Base
	...
	has_many :provider_open_days, dependent: :destroy
	...
	#Check if provider is open between two datetimes (on same day)
	def is_open(start_time, end_time)

	    correct_time = false

	    #Check for provider_times first
	    if self.provider_times.where(day_id: start_time.cwday).count > 0
	      self.provider_times.where(day_id: start_time.cwday).each do |provider_time|
	        if (provider_time.open.strftime('%R') <= start_time.strftime("%R") && provider_time.close.strftime('%R') >= end_time.strftime('%R'))
	          correct_time = true
	          break
	        end
	      end
	    ...
	    #If there was no provider_time, check for an open_day
	    if !correct_time
	      if (self.provider_open_days.where('start_time <= ? and end_time >= ?', start_time, end_time).count > 0)
	        correct_time = true
	      end
	    end      
