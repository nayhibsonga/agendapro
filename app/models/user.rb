class User < ActiveRecord::Base
  ...
  # :confirmable, :lockable, :timeoutable and :omniauthable
	...
	has_many :client_comments, dependent: :nullify
	...
	def send_welcome_mail
		unless Role.where(name: ["Staff", "Recepcionista", "Recepcionista (sin edición)", "Admnistrador Local", "Staff (sin edición)"]).include? self.role
	...
	def location_company_users
		if Role.where(:name => ["Administrador Local","Recepcionista","Recepcionista (sin edición)"]).include? self.role
	...
	def is_disabled
		if self.role_id == Role.find_by_name("Recepcionista").id || self.role_id == Role.find_by_name("Recepcionista (sin edición)").id
