    ...
    all.order(:created_at).each do |sparkpost_email_log|
      ...
      json.each do |event|
        ...
        if message_event.present? && message_event["rcpt_meta"].present? && SparkpostStatus.find_by_event_type(message_event["type"]).present?
          ...
          elsif message_event["rcpt_meta"]["campaign_id"].present? && Email::Content.find_by(id: message_event["rcpt_meta"]["campaign_id"]) && Client.find_by(email: message_event["rcpt_to"], company_id: Email::Content.find_by(id: message_event["rcpt_meta"]["campaign_id"]).company.id) && SparkpostStatus.find_by_event_type(message_event["type"]).present?
