...
function getInventory()
{
	...
	if(location_id == "0")
	{
		...
		$.ajax({
			...
			type: 'get',
			data: {category: category, brand: brand, display: display, searchInput: searchInput, order: order, status: status},
			success: function(response)
			{
	...
	else
	{
		...
		$.ajax({
			url: '/inventory',
			type: 'get',
			data: {id: location_id, category: category, brand: brand, display: display, searchInput: searchInput, order: order, status: status},
			success: function(response)
			{
				$("#locationInventory").empty();
				$("#locationInventory").append(response);
...
$(function() {
	...
	$('#toggleProducts').on('click', function(){
		if($('#active-filter').val() == "active")
		{
			$('#active-filter-text').html("Ver activos");
			$('#active-filter').val("inactive");
		}
		else
		{
			$('#active-filter-text').html("Ver inactivos");
	...
	$('body').on("ajax:success", ".deactivate-product-btn", function(e, data, status, xhr){
	    console.log(data);
	    var product_id = data.id;
	    $('.inventoryRow[product_id="' + product_id + '"]').remove();
	    $('.inventoryProductRow[product_id="' + product_id + '"]').remove();
	    triggerSuccess("Producto desactivado correctamente");
	...
	$('body').on("ajax:success", ".activate-product-btn", function(e, data, status, xhr){
	    console.log(data);
	    var product_id = data.id;
	    $('.inventoryRow[product_id="' + product_id + '"]').remove();
	    $('.inventoryProductRow[product_id="' + product_id + '"]').remove();
	    triggerSuccess("Producto reactivado correctamente");
	}).on("ajax:error", function(e, xhr, status, error){
	    console.log(xhr.responseText)
	    swal({
	      title: "Error",
