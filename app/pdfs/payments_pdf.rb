class PaymentsPdf < Prawn::Document
	...
	def company_info(company)
		y_position = cursor
		bounding_box([180, y_position], width: 260, height: 30) do
			text "<b>Comprobante #{company.name}</b>",
				:inline_format => true
		end

		move_down 10

		if !company.billing_info.nil?
			company_rows = []
			company_rows << ["Razón social: ", "#{company.billing_info.name}"]
			company_rows << ["Rut empresa: ", "#{company.billing_info.rut}"]
			company_rows << ["Dirección: ", "#{company.billing_info.address}"]
			company_rows << ["Giro: ", "#{company.billing_info.sector}"]
			company_rows << ["Responsable: ", "#{company.billing_info.contact}"]
			company_rows << ["Email: ", "#{company.billing_info.email}"]
			company_rows << ["Teléfono: ", "#{company.billing_info.phone}"]
			company_rows << ["Información Adicional: ", "#{company.billing_info.additional_info}"]
