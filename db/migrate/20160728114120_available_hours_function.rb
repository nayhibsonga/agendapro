  ...
  def self.up
	 execute <<-__EOI
		...
		BEGIN
		  ...
		  RAISE NOTICE 'Checking for % to %, with provider %', start_date, end_date, provider_id;
		  ...
		  --Added location_open_days checks
		  IF not exists(select id from location_times where location_times.location_id = local_id AND location_times.day_id = day AND location_times.open <= (select "time"(start_date)) AND location_times.close >= (select "time"(end_date))) THEN
		  	IF not exists(select id from location_open_days where location_open_days.location_id = local_id AND location_open_days.start_time <= start_date AND location_open_days.end_time >= end_date) THEN
		      RAISE NOTICE 'Fails location times and open days';
		      return false;
		    END IF;
		  END IF;
		  ...
		  IF (admin = false) THEN
		      ...
		      RAISE NOTICE 'Fails limits';
			  	return false;
			  END IF;
		  ELSE
		  	IF start_date < current_start THEN
		      RAISE NOTICE 'Fails current_start';
			  	return false;
		    END IF;
		  END IF;

		  --Check provider_times
		  --Added provider_open_days checks
		  IF not exists (select id from provider_times where provider_times.service_provider_id = provider_id AND provider_times.day_id = day AND provider_times.open <= (select "time"(start_date)) AND provider_times.close >= (select "time"(end_date))) THEN
		    ...
		    IF not exists (select id from provider_open_days where provider_open_days.service_provider_id = provider_id AND provider_open_days.start_time <= start_date AND provider_open_days.end_time >= end_date ) THEN
		        RAISE NOTICE 'Fails provider_times and open_days';
		        return false;
		    ELSE
		      found_id := (select id from provider_open_days where provider_open_days.service_provider_id = provider_id AND provider_open_days.start_time <= start_date AND provider_open_days.end_time >= end_date limit 1);
		      RAISE NOTICE 'Found %', found_id;
		    END IF;
		  END IF;
		  ...
		  IF (select time_restricted from services where services.id = serv_id limit 1) THEN
		      ...
		      RAISE NOTICE 'Fails service_times';
		  		return false;
		  	END IF;
		  END IF;
		  ...
		  IF exists (select id from bookings where bookings.service_provider_id = provider_id AND bookings.service_id <> serv_id AND bookings.status_id != 5 AND (bookings.is_session = false OR (bookings.is_session = true AND bookings.is_session_booked = true)) AND ((start_date, end_date) OVERLAPS (bookings."start", bookings."end"))) THEN
		    RAISE NOTICE 'Fails cross_bookings';
		    return false;
		  END IF;
		  IF (select group_service from services where id = serv_id) = false THEN
		      ...
		      RAISE NOTICE 'Fails group_service';
		      return false;
		    END IF;
		  ELSE
		  ...
		  IF exists(select resource_id from service_resources where service_resources.service_id = serv_id) THEN
		    ...
		    FOR r_id IN (select resource_id from service_resources where service_resources.service_id = serv_id) LOOP
		      ...
		      IF NOT exists(select resource_id from resource_locations where resource_locations.resource_id = r_id AND resource_locations.location_id = local_id) THEN
		        RAISE NOTICE 'Fails resources existance';
		        return false;
		      END IF;
		...
		BEGIN
		  ...
		  SELECT INTO company_setting * from company_settings where company_settings.company_id = service.company_id limit 1;
		  ...
		  ELSE
		    ...
		    IF service.has_sessions = FALSE THEN
		      ...
		      IF (service.has_time_discount AND company_setting.promo_offerer_capable AND service.time_promo_active) THEN
		        ...
		        IF exists(select id from promos where promos.day_id = day and promos.service_promo_id = service.active_service_promo_id and promos.location_id = local_id) THEN
		          --service_promo := (select * from service_promos where id = service.active_service_promo_id);
		          SELECT INTO service_promo * from service_promos where service_promos.id = service.active_service_promo_id;
		          SELECT INTO promo * from promos where promos.day_id = day and promos.service_promo_id = service.active_service_promo_id and promos.location_id = local_id;
		    ...
		    ELSE
		      ...
		      IF (service.has_treatment_promo AND company_setting.promo_offerer_capable AND service.time_promo_active) THEN
		        ...
		        IF exists(select id from treatment_promos where treatment_promos.service_id = service.id) AND exists(select id from treatment_promo_locations where treatment_promo_locations.location_id = local_id and treatment_promo_locations.treatment_promo_id = service.active_treatment_promo_id) THEN
		...
		DECLARE
		  ...
		  book_diff int;
		  break_diff int;
		  provider_time_diff int;

		  book_blocking timestamp;
		  break_blocking timestamp;
		  provider_time_blocking time;
		  provider_open timestamp;

		  open_day_gap timestamp;
		  open_day_blocking timestamp;
		  open_day_diff int;
		...
		BEGIN
		  ...
		  duration := 0;--(select sum(services.duration) from services where id = ANY(service_ids));
		  ...
		  IF not exists(select id from location_times where location_times.location_id = local_id AND location_times.day_id = day) THEN
		    ...
		    IF not exists(select id from location_open_days where location_open_days.location_id = local_id AND location_open_days.start_time >= start_date AND location_open_days.start_time <= end_date) THEN
		  ...
		  LOOP
		    ...
		    <<staff_loop>>
		      ...
		      IF EXISTS(select open from provider_times where service_provider_id = ANY(current_service_providers_ids) and day_id = day order by open limit 1) THEN
		        --Nothing
		      ELSE
		        IF not exists (select id from provider_open_days where provider_open_days.service_provider_id = ANY(current_service_providers_ids) AND provider_open_days.start_time >= start_date AND provider_open_days.end_time <= end_date ) THEN
		          RAISE NOTICE 'No open for provider';
		          RETURN;
		          EXIT staff_loop;
		        ELSE
		          min_open := (select start_time from provider_open_days where provider_open_days.service_provider_id = ANY(current_service_providers_ids) AND provider_open_days.start_time >= start_date AND provider_open_days.end_time <= end_date order by start_time limit 1);
		        END IF;
		      END IF;
		     ...
		     --RAISE NOTICE 'BEFORE STATE: service_staff_pos: % - allows_optimization: % - last_check % - allows_overlap %', service_staff_pos, allows_optimization, last_check, allows_overlap;
		      ...
		      ELSE
		        ...
		        IF (allows_optimization AND last_check) THEN
		          ...
		          --Look for gaps, that is bookings, breaks, time_closures or open_day_ends
		          --that could overlap current time window
		          --to see if anything fits before next
		          --booking, break, time close or open_day_end

		          --IMPORTANT
		          --When they are times, add date
		          RAISE NOTICE 'Looking for gaps with dtp %', dtp;

		          book_gap := (select bookings.start from bookings where service_provider_id = ANY(first_providers_ids) AND ( (bookings.start, bookings.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) AND (bookings.service_id NOT IN (SELECT services.id from services where services.group_service = TRUE AND services.company_id = comp_id)) order by bookings.start limit 1);

		          break_gap := (select provider_breaks.start from provider_breaks where service_provider_id = ANY(first_providers_ids)  AND ( (provider_breaks.start, provider_breaks.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) order by provider_breaks.start limit 1);

		          time_provider_time_gap := (select provider_times.close from provider_times where service_provider_id = ANY(first_providers_ids) AND (select "time"(dtp)) <= provider_times.close AND (select "time"((dtp + interval '1' minute * first_duration))) >= provider_times.open and day_id = day order by provider_times.close limit 1);
		          ...
		          open_day_gap := (select provider_open_days.end_time from provider_open_days where provider_open_days.service_provider_id = ANY(first_providers_ids) AND ((provider_open_days.start_time, provider_open_days.end_time) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration))) order by provider_open_days.end_time limit 1);

		          -- Check if there is a provider close

		          IF exists(select provider_times.close from provider_times where service_provider_id = ANY(first_providers_ids) AND (select "time"(dtp)) <= provider_times.close AND (select "time"((dtp + interval '1' minute * first_duration))) >= provider_times.open and day_id = day order by provider_times.close limit 1) THEN

		            provider_close := date_trunc('day', dtp) + interval '1h' * date_part('hour', time_provider_time_gap) + interval '1' minute * date_part('minute', time_provider_time_gap);
		            ...
		            IF (dtp < provider_close) AND (provider_close < (dtp + interval '1' minute * duration)) THEN
		              ...
		              RAISE NOTICE 'Provider time gap is %', gap_diff;

		              IF (gap_diff > time_gap) AND (gap_diff < duration) THEN
		                time_gap := gap_diff;
		              END IF;
		          ...
		          -- Check if there is an open_day end
		          IF exists(select provider_open_days.end_time from provider_open_days where provider_open_days.service_provider_id = ANY(first_providers_ids) AND ((provider_open_days.start_time, provider_open_days.end_time) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration))) order by provider_open_days.end_time limit 1) THEN
		            gap_diff := (select extract( epoch from (open_day_gap - dtp) ) / 60);
		            RAISE NOTICE 'Open day gap is %', gap_diff;
		            IF (gap_diff > time_gap) AND (gap_diff < duration) THEN
		              time_gap := gap_diff;
		            END IF;
		          END IF;

		          IF exists(select bookings.start from bookings where service_provider_id = ANY(first_providers_ids) AND ( (bookings.start, bookings.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) AND (bookings.service_id NOT IN (SELECT services.id from services where services.group_service = TRUE AND services.company_id = comp_id)) order by bookings.start limit 1) THEN
		            gap_diff := (select extract( epoch from (book_gap - dtp) ) / 60);
		            RAISE NOTICE 'Booking gap is %', gap_diff;
		            IF (gap_diff > time_gap) AND (gap_diff < duration) THEN
		              time_gap := gap_diff;
		            END IF;
		          END IF;

		          IF exists(select provider_breaks.start from provider_breaks where service_provider_id = ANY(first_providers_ids)  AND ( (provider_breaks.start, provider_breaks.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) order by provider_breaks.start limit 1) THEN
		            gap_diff := (select extract( epoch from (break_gap - dtp) ) / 60);
		            RAISE NOTICE 'Break gap is %', gap_diff;
		            IF (gap_diff > time_gap) AND (gap_diff < duration) THEN
		                time_gap := gap_diff;
		              END IF;
		          END IF;
		        ...
		        --When there is no gap or it is bigger than the duration, calculate the step to an ending time of a booking or break, or starting time of next provider_time or open_day.
		        --Grab the smallest difference.
		        IF ((allows_optimization) AND ((time_gap = 0) OR (time_gap > duration))) THEN

		          RAISE NOTICE 'Looking for smallest_diff with dtp %', dtp;

		          book_blocking := (select bookings.end from bookings where service_provider_id = ANY(first_providers_ids) AND ( (bookings.start, bookings.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) AND (bookings.service_id NOT IN (SELECT services.id from services where services.group_service = TRUE AND services.company_id = comp_id)) order by bookings.end limit 1);

		          break_blocking := (select provider_breaks.end from provider_breaks where service_provider_id = ANY(first_providers_ids)  AND ( (provider_breaks.start, provider_breaks.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) order by provider_breaks.end limit 1);

		          provider_time_blocking := (select provider_times.open from provider_times where service_provider_id = ANY(first_providers_ids) AND (select "time"(dtp)) <= provider_times.close AND (select "time"((dtp + interval '1' minute * first_duration))) >= provider_times.open and day_id = day order by provider_times.open desc limit 1);

		          open_day_blocking := (select provider_open_days.start_time from provider_open_days where provider_open_days.service_provider_id = ANY(first_providers_ids) AND ((provider_open_days.start_time, provider_open_days.end_time) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration))) order by provider_open_days.start_time desc limit 1);
		          ...
		          IF exists(select bookings.end from bookings where service_provider_id = ANY(first_providers_ids) and bookings.end >= dtp and bookings.start <= (dtp + interval '1' minute * first_duration) AND (bookings.service_id NOT IN (SELECT services.id from services where services.group_service = TRUE AND services.company_id = comp_id)) order by bookings.end limit 1) THEN
		            ...
		            IF (book_diff < smallest_diff) AND (book_diff > 0) THEN
		              smallest_diff := book_diff;
		            END IF;

		          END IF;

		          IF exists(select provider_breaks.end from provider_breaks where service_provider_id = ANY(first_providers_ids)  AND ( (provider_breaks.start, provider_breaks.end) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration)) ) order by provider_breaks.end limit 1) THEN

		            break_diff := (select (extract (epoch from (break_blocking - dtp) ) ) / 60);

		            IF (break_diff < smallest_diff) AND (break_diff > 0) THEN
		              smallest_diff := break_diff;
		            END IF;

		          END IF;

		          IF exists(select provider_times.open from provider_times where service_provider_id = ANY(first_providers_ids) AND (select "time"(dtp)) <= provider_times.close AND (select "time"((dtp + interval '1' minute * first_duration))) >= provider_times.open and day_id = day order by provider_times.open desc limit 1) THEN

		            provider_open := date_trunc('day', dtp) + interval '1h' * date_part('hour', provider_time_blocking) + interval '1' minute * date_part('minute', provider_time_blocking);

		            --Check if provider_close is between current time window. If so, assign time_gap.

		            IF (dtp < provider_open) AND (provider_open < (dtp + interval '1' minute * duration)) THEN

		              provider_time_diff := (select extract( epoch from (provider_open - dtp) ) / 60);

		              IF (provider_time_diff < smallest_diff) AND (provider_time_diff > 0) THEN
		                smallest_diff := provider_time_diff;
		              END IF;
		          ...
		          IF exists(select provider_open_days.start_time from provider_open_days where provider_open_days.service_provider_id = ANY(first_providers_ids) AND ((provider_open_days.start_time, provider_open_days.end_time) OVERLAPS (dtp, (dtp + interval '1' minute * first_duration))) order by provider_open_days.start_time desc limit 1) THEN
		            open_day_diff := (select (extract (epoch from (open_day_blocking - dtp) ) ) / 60);

		            IF (open_day_diff < smallest_diff) AND (open_day_diff > 0) THEN
		              smallest_diff := open_day_diff;
		            END IF;
		          END IF;
		        ...
		        RAISE NOTICE 'Smalles diff was %', smallest_diff;
		        RAISE NOTICE 'Time gap was %', time_gap;

		        IF (allows_optimization AND (time_gap > 0) AND (time_gap <= duration)) THEN
