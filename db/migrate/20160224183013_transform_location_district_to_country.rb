class TransformLocationDistrictToCountry < ActiveRecord::Migration
  def change
    Location.all.each do |location|
      ...
      country = region.present? && Country.find(Region.find(City.find(District.find(location.district_id).city_id).region_id).country_id) ? Country.find(Region.find(City.find(District.find(location.district_id).city_id).region_id).country_id) : location.company.country
