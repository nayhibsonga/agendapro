...
BEGIN
  ...
  IF exists(select resource_id from service_resources where service_resources.service_id = serv_id) THEN
    ...
    FOR r_id IN (select resource_id from service_resources where service_resources.service_id = serv_id) LOOP
      ...
      resources_max := (select sum(quantity) from resource_locations where resource_locations.resource_id = r_id);

      -- Get used resources from not group service location bookings
      resources_count1 := ( select count(bookings_id) from bookings where bookings.service_id in (select id from service where group_service = false) AND bookings.location_id = local_id AND bookings.status_id != 5 AND (bookings.is_session = false OR (bookings.is_session = true AND bookings.is_session_booked = true)) AND not(provider_breaks."end" <= start_date OR end_date <= provider_breaks."start") AND bookings.service_id in (select serv_id from service_resources where service_resources.resource_id = r_id) );
      -- Get used resources from group service location bookings
      resources_count2 := ( select count(*) from ( select DISTINCT(id) from bookings where bookings.service_id in (select id from service where group_service = true) AND (bookings.service_id != serv_id OR bookings.service_provider_id != provider_id) AND bookings.location_id = local_id AND bookings.status_id != 5 AND (bookings.is_session = false OR (bookings.is_session = true AND bookings.is_session_booked = true)) AND not(provider_breaks."end" <= start_date OR end_date <= provider_breaks."start") AND bookings.service_id in (select serv_id from service_resources where service_resources.resource_id = r_id) ) as rc_query );
